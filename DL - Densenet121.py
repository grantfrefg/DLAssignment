...
##Resnet model
basemodel = DenseNet121(input_shape=(dim,dim,3), weights = 'imagenet', include_top=False)
base = basemodel.output 
base = GlobalAveragePooling2D()(base)

dense = Dense(512, activation = 'relu')(base)
...
##Save path
final_weights_path=path + "model_weightsXception-{epoch:02d}-{val_acc:.2f}.hdf5"
callbacks_list = [
...
final_weights_path = os.path.join(os.path.abspath(path), 'model_weightsXception-09-0.92.hdf5')
...
XceptionPred = np.mean(np.hstack([TTA1Pred, TTA2Pred, TTA3Pred, TTA4Pred, TTA5Pred]), axis = 1)

##Get class label
XceptionPredC = np.argmax(XceptionPred, axis = 1)
...
print(accuracy_score(valy, XceptionPredC))
...
np.save(path + 'XceptionPredC', XceptionPredC)

np.save(path + 'TTA1PredXc', TTA1Pred)

np.save(path + 'TTA2PredXc', TTA2Pred)

np.save(path + 'TTA3PredXc', TTA3Pred)

np.save(path + 'TTA4PredXc', TTA4Pred)

np.save(path + 'TTA5PredXc', TTA5Pred)


###Ensembling models

ResNetPred = np.load(path + 'ResNetPred.npy')
VGGPred = np.load(path + 'VGGPred.npy')

VGGPredC = np.argmax(VGGPred, axis = 1)
print(accuracy_score(valy, VGGPredC))
...
Ensemble = np.mean((XceptionPred, VGGPred, ResNetPred), axis = 0)
Ensemblepred = np.argmax(Ensemble, axis = 1)
accuracy_score(valy, Ensemblepred)