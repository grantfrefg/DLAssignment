...
def GeneratorAug(files, labels, batchsize, img_dim):
    ...
    while 1:
        for i in range(0, len(files), batchsize):
            batchX = files[i:i+batchsize]
            batchY = np.array(labels[i:i+batchsize][:])
            batchX = np.array([Augment(cv2.imread(fname), img_dim, 5, 0.01, 0.01, 10) for fname in batchX])
            yield(batchX, batchY)
...
##Resnet model
basemodel = VGG19(input_shape=(dim,dim,3), weights = 'imagenet', include_top=False)
base = basemodel.output 
base = GlobalAveragePooling2D()(base)

dense = Dense(256, activation = 'relu')(base)
...
##Save path
final_weights_path=path + "model_weightsVGG19-{epoch:02d}-{val_acc:.2f}.hdf5"
callbacks_list = [
...
final_weights_path = os.path.join(os.path.abspath(path), 'model_weightsVGG19-32-0.92.hdf5')
...
VGGPred = np.mean(np.hstack([TTA1Pred, TTA2Pred, TTA3Pred, TTA4Pred, TTA5Pred]), axis = 1)
...
np.save(path + 'VGGPred', VGGPred)

np.save(path + 'TTA1Pred', TTA1Pred)

np.save(path + 'TTA2Pred', TTA2Pred)

np.save(path + 'TTA3Pred', TTA3Pred)

np.save(path + 'TTA4Pred', TTA4Pred)

np.save(path + 'TTA5Pred', TTA5Pred)
